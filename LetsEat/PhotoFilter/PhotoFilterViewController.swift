...
import UIKit
import AVFoundation
import MobileCoreServices /// Covers the programming interfaces for working with the system-declared uniform types.
...
private extension PhotoFilterViewController {
    ...
    /// Does a basic setup for the collection view that will be used to display filters.
    ...
    /// Check the user authorization status for the use of the camera.
    func checkSource() {
...
extension PhotoFilterViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    /// Triggered if the user selects a picture from either the camera or the photo library.
    ...
    /// Create a standard `UIImagePickerController` instance when the Camera button is tapped.
    ...
    /// Create a large and small image, which will be assigned to the `PhotoFilterViewController` image and thumbnail properties and dismiss the image picker/
...
extension PhotoFilterViewController: ImageFiltering, ImageFilteringDelegate {
    ...
    /// Apply the filter the user selected for the image stored in the `PhotoFilterViewController` image property, and the result is assigned to the `imgExample` outlet so that it is visible on screen.
...
extension PhotoFilterViewController: UICollectionViewDelegate {
    ...
    /// Called when a user taps a cell in the collection view to pass the `FilterIte` to `filterSelected(item:)`.
